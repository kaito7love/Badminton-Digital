...
The first deliverable must be an **audit of the existing Badminton Digital codebase against this document**. (Done: Initial Audit Report completed).
...
# 28. Phased Implementation Strategy (Post-Audit)

Dựa trên kết quả Audit, hệ thống hiện tại cần chuẩn hóa Data Model trước khi áp dụng Socket.IO. Việc nâng cấp sẽ được thực hiện theo 3 giai đoạn:

### Phase 1: Chuẩn hóa Database Models & REST API (P0) — [COMPLETED]
- [x] **Refactor `Court` Model:** Đổi `status` thành `ENUM('active', 'maintenance', 'inactive')`. Đã chuyển các trạng thái cũ sang `active`.
- [x] **Refactor `CourtSession` Model:** Cập nhật `status` hỗ trợ `('playing', 'completed', 'cancelled', 'closed')`.
- [x] **Refactor Services & Controllers:** Viết lại `CourtService.js` để tính toán trạng thái động (`state`: `AVAILABLE`, `PLAYING`, `MAINTENANCE`, `INACTIVE`). `openCourt` và `closeCourt` không còn ghi đè trạng thái bảng `Court`.
- [x] **Frontend Synchronization:** Cập nhật `mapStatus` và `mapCourt` trong `CourtsPage.jsx` để nhận diện cấu trúc trạng thái động mới.
- [x] **Unit Testing:** Đã viết unit test trong `courtService.test.js`. Toàn bộ 7/7 test suite của backend đều **PASS**.

### Phase 2: Setup Socket.IO & Branch Isolation (P1) — [NEXT STEP]
1. Cài đặt `socket.io` (Backend) và `socket.io-client` (Frontend).
2. Xây dựng Middleware verify JWT và join room theo `branchId` (`socket.join("branch:" + branchId)`).
3. Viết cơ chế emit event (Chỉ emit SAU KHI database transaction commit thành công).

### Phase 3: Frontend Integration & State Sync (P1) — [PENDING]
1. Fetch snapshot từ REST API khi load app hoặc reconnect.
2. Lắng nghe và apply các realtime events (`court.session.started`, `court.session.completed`, v.v.).
3. Chạy live timer ở client, không phụ thuộc ping từ server.
4. Xử lý cơ chế đồng bộ lại khi mất kết nối mạng.
