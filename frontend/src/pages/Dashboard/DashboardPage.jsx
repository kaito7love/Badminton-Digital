    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400 max-w-2xl">Theo dõi tình trạng 8 sân cầu lông, doanh thu thực tế, lịch đặt sân & cảnh báo kho vật tư.</p>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-4 lg:grid-cols-2">
        {stats.map((item, idx) => (
          <div key={item.label || idx} className="rounded-3xl border border-slate-200 bg-white dark:border-slate-800/80 dark:bg-slate-900/80 p-6 shadow-2xl backdrop-blur-xl hover:border-emerald-500/40 transition">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white dark:border-slate-800/80 dark:bg-slate-900/80 p-6 md:p-8 shadow-2xl backdrop-blur-xl">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-bold text-slate-900 dark:text-slate-100">Lượt Đặt Sân Trong Tuần</h2>
            </div>
            <span className="rounded-full bg-emerald-500/10 border border-emerald-500/30 px-3.5 py-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400">7 Ngày Qua</span>
          </div>
          ...
          <div className="h-[320px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="name" tick={{ fill: chartColors.axisTick, fontSize: 12 }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fill: chartColors.axisTick, fontSize: 12 }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ backgroundColor: chartColors.tooltipBg, borderRadius: 16, border: `1px solid ${chartColors.tooltipBorder}`, color: chartColors.tooltipText }} />
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white dark:border-slate-800/80 dark:bg-slate-900/80 p-6 shadow-2xl backdrop-blur-xl">
            ...
            <div className="space-y-3">
              {['Sân 01 - BWF Arena', 'Sân 02 - VIP Lounge', 'Sân 03 - Pro Studio', 'Sân 04 - Standard Arena'].map((court) => (
                <div key={court} className="flex items-center justify-between rounded-2xl bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800/60 p-3.5">
                  <div>
                    <p className="text-sm font-bold text-slate-900 dark:text-white">{court}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">Trạng thái: <span className="text-emerald-600 dark:text-emerald-400 font-semibold">Sẵn sàng</span></p>
                  </div>
                  <span className="rounded-full bg-emerald-500/10 border border-emerald-500/30 px-3 py-1 text-xs font-bold text-emerald-600 dark:text-emerald-400">Hoạt động</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-amber-500/30 bg-white dark:bg-slate-900/80 p-6 shadow-2xl backdrop-blur-xl">
            ...
            <div className="space-y-2.5 text-xs font-medium text-slate-600 dark:text-slate-300">
              <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <span>🥤 Nước giải khát Pocari / RedBull:</span>
                <span className="font-bold text-amber-600 dark:text-amber-400">Còn 12 chai</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <span>🏸 Quả cầu lông Yonex AS-50:</span>
                <span className="font-bold text-amber-600 dark:text-amber-400">Còn 5 ống</span>
              </div>
              <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
                <span>🎾 Vợt Astrox 99 Cho Thuê:</span>
                <span className="font-bold text-emerald-600 dark:text-emerald-400">Đủ 8 chiếc</span>
              </div>
            </div>
          </div>
