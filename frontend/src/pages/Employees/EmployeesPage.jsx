...
export default function EmployeesPage() {
  ...
  const fetchEmployees = async (targetPage = page) => {
    try {
      setLoading(true);
      const res = await employeeService.getAllEmployees({ page: targetPage });
  ...
  if (loading) return <div className="p-8 text-slate-600 dark:text-slate-300">⏳ Đang tải danh sách nhân viên...</div>;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-sm uppercase tracking-[0.24em] text-emerald-600 dark:text-emerald-400 font-medium">Employees</p>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Quản lý nhân viên</h1>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {employees.map((emp) => {
              ...
              return (
                <div
                  key={emp.id}
                  className="rounded-3xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/80 p-5 flex flex-col justify-between space-y-4 hover:border-slate-300 dark:hover:border-slate-700 transition"
                >
                  <div>
                    <div className="flex items-start justify-between">
                      <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">{role}</span>
      ...
      <Pagination meta={meta} onPageChange={setPage} itemLabel="nhân viên" />
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          {!editingEmployee && (
            <>
              <div>
                <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Mật khẩu *</label>
                <input
                  type="password"
                  required
                  minLength={6}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Chức vụ / Vị trí</label>
              <select
            ...
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Ca làm việc</label>
              <select
